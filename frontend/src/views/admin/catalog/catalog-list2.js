...
import {catalog2DataFetch,selectedCatalog2StatusUpdate,selectedCatalog2Delete,getDecodeJwt} from "../../../service/web/userService";
...
const UserList =() =>{
   ...
   async function getCatalog1DataList(){
      ...
      const {data:company} =await catalog2DataFetch(auth.companyId);
   ...
   const  handleToggle=async (item,idx)=>{
      ...
      //update db api
      try{
         const response = await selectedCatalog2StatusUpdate(item.id)
         if(response){
            let newArr = [...catalog1List];
                   ...
                   <Card>
                      <Card.Header className="d-flex justify-content-between">
                         <div className="header-title">
                            <h4 className="card-title">Catalog 2</h4>
                         </div>
                      </Card.Header>
